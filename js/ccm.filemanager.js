	...
	*/
	...
	if (!bobj) {
		...
		if ($(obj).attr('ccm-file-manager-can-edit') == '1') {
			html += '<li><a class="ccm-icon dialog-launch" dialog-modal="false" dialog-width="90%" dialog-height="70%" dialog-title="' + ccmi18n_filemanager.edit + '" id="menuEdit' + fID + '" href="' + CCM_DISPATCHER_FILENAME + '/tools/packages/image_cropper/files/edit?fID=' + fID + '&searchInstance=' + searchInstance + '&ocID=' + CCM_CID + '"><span style="background-image: url(' + CCM_IMAGE_PATH + '/icons/edit_small.png)">'+ ccmi18n_filemanager.edit + '<\/span><\/a><\/li>';
		}
